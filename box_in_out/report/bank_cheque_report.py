...
class ReportProductSale(models.AbstractModel):
    _name = "report.box_in_out.box_cheques_report_in_out"

    @api.model
    def _get_report_values(self, docids, data=None):
        date_from = data["form"]["date_from"]
        date_to = data["form"]["date_to"]
        # date_to = data["form"]["date_to"].strptime('%Y-%m-%%d')
        ...
        lines=[]
        for rec in move_lines:
            amount_debit,amount_credit,payment_amount,cheque_amount,other=0,0,0,0,0
            if rec.journal_id not  in journal_list:
                journal_list.append(rec.journal_id)
            if  rec.account_id == rec.journal_id.default_debit_account_id :
                if rec.payment_id:
                    payment_amount = rec.debit
                elif rec.cheques ==True:
                    cheque_amount = rec.debit
                elif rec.debit>0:
                    other=rec.debit


                lines.append({'partner_id': rec.partner_id, 'journal_id': rec.journal_id, 'date': rec.date
                                 , 'payment_amount': payment_amount, 'cheque_amount': cheque_amount,'other':other})
